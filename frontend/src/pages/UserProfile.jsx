...
const UserProfile = () => {
  ...
  const navigate = useNavigate();
    ...
    const fetchProfile = async()=>{
      try {
        const newprofile = await axios.get(`${URI}/api/profile/oneprofile/${email}`)
    ...
    const handleReviewSubmit = async(e)=>{
      ...
      if(newReview.length === 0){
        setReviewMessage('Error : Enter something to post the review')
        return
      }
      try {
        console.log(user.email)
        const newprofile = await axios.patch(`${URI}/api/profile/updatereviews/${user.email}`,{review:newReview,rating:newRating})
        console.log(newprofile.data.message)
        setNewReview('')
        setReviewMessage('')
      } catch (error) {
        ...
        <div className="min-h-screen bg-gray-50 text-gray-800">
            ...
            <div className="container mx-auto px-4 py-8 pt-12">
                <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-lg p-8 space-y-8">
                    ...
                    {/* Reviews Section */}
                    <div>
                        <h3 className="text-2xl font-bold text-gray-900 mb-4 border-b pb-2">{user.name.charAt(0).toUpperCase() + user.name.slice(1)} Customer Reviews</h3>
                    ...
                    <div className="pt-8 border-t border-gray-200">
                        <h3 className="text-2xl font-bold text-gray-900 mb-4">Your experience with this driver : {user.name.charAt(0).toUpperCase() + user.name.slice(1)}</h3>
                        <form onSubmit={handleReviewSubmit} className="space-y-4">
                            <div>
                                <label htmlFor="appRating" className="block text-left text-gray-800 font-medium mb-2">
                                    Your Rating
                                </label>
                                <select
                                    id="appRating"
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    placeholder="Share your thoughts about the travel and experience with this driver"
                                ></textarea>
                            </div>
                            {reviewMessage && (
